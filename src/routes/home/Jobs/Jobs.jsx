...
const JobsQueue = () => {
  ...
  const [statusSelected, setStatusSelected] = useState(
    JSON.parse(localStorage.getItem("statusSelected")) || {
      id: "All",
      name: "All Open Jobs",
    }
  );
  const [sortSelected, setSortSelected] = useState(
    JSON.parse(localStorage.getItem("sortSelected")) || {
      id: "requestDate",
      name: "Request Date",
    }
  );
  ...
  const [projectManagerSelected, setProjectManagerSelected] = useState(
    JSON.parse(localStorage.getItem("projectManagerSelected")) || {
      id: "All",
      name: "All",
    }
  );
  ...
  useEffect(() => {
    localStorage.setItem(
      ...
      JSON.stringify(projectManagerSelected)
    );
  }, [projectManagerSelected]);
                          ...
                          <div className="w-full grid xl:grid-cols-2 lg:grid-cols-2 md-grid-cols-2 xs:grid-cols-1">
                            ...
                            <div className="xl:text-right lg:text-right md:text-right xs:text-left sm:text-left">
                              <div className="flex gap-2 xl:justify-end lg:justify-end xs:justify-start sm:justify-start">
                                {job.isDueToday && (
                                  <p
                                    className={`inline-flex text-xs rounded-md py-1 px-2 text-red-500 border border-red-500 font-semibold`}
                                  >
                                    DUE TODAY
                                  </p>
                                )}
