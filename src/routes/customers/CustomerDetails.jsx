...
const CustomerDetails = () => {
    ...
    const updateShowSpendingInfo = async() => {
        ...
        try {
          const { data } = await api.updateCustomerSetting(customerDetails.settings.id, request)
          
          const updatedCustomerDetails = {...customerDetails,
                                          settings: {...customerDetails.settings,
               ...
               <div className="mx-auto mt-6 max-w-5xl px-4 sm:px-6 lg:px-8">
                  <dl className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
                    <div className="sm:col-span-1">
                        ...
                        <dd className="mt-1 text-sm text-gray-900">{customerDetails?.phone_number ? formatPhoneNumber(customerDetails.phone_number) : 'Not specified'}</dd>
                    </div>
                    <div className="sm:col-span-1">
                        <dt className="text-sm font-medium text-gray-500">Email</dt>
                        <dd className="mt-1 text-sm text-gray-900">{customerDetails?.emailAddress ? customerDetails?.emailAddress : 'Not specified'}</dd>
                    </div>
                    <div className="sm:col-span-1">
                    ...
                    <div className="sm:col-span-1">
                        ...
                        <dd className="mt-1 text-sm text-gray-900">
                          {customerDetails?.settings?.retainer_amount ? '$' + customerDetails?.settings?.retainer_amount.toLocaleString() : 'Not specified'}</dd>
                    </div>
                    <div className="sm:col-span-1">
                        <dt className="text-sm font-medium text-gray-500">Active</dt>
                        <dd className="mt-1 text-sm text-gray-900">{customerDetails?.active ? 'Yes' : 'No'}</dd>
                    </div>
                    <div className="sm:col-span-2">
                      <dt className="text-sm font-medium text-gray-500">About</dt>
                      <dd
                        className="mt-1 max-w-prose space-y-5 text-sm text-gray-900"
                      >{customerDetails?.about ? customerDetails?.about : 'Not specified'}</dd>
                    </div>
                  </dl>
                </div>
                  ...
                  <div className="">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-500">
                        Control what all users associated with this customer can see and do.
                      </p>
                    </div>
                    <ul role="list" className="mt-2 divide-y divide-gray-200">
                      <Switch.Group as="li" className="flex items-center justify-between py-4">
                        ...
                        <Switch
                          ...
                          className={classNames(
                            customerDetails?.settings?.show_spending_info ? 'bg-red-500' : 'bg-gray-200',
                        ...
                        >
                          <span
                            aria-hidden="true"
                            className={classNames(
                              customerDetails?.settings?.show_spending_info ? 'translate-x-5' : 'translate-x-0',
                      ...
                      <Switch.Group as="li" className="flex items-center justify-between py-4">
                        ...
                        <Switch
                          ...
                          className={classNames(
                            customerDetails?.settings?.allow_cancel_job ? 'bg-red-500' : 'bg-gray-200',
                            'relative ml-4 inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2'
                          )}
                        >
                          <span
                            aria-hidden="true"
                            className={classNames(
                              customerDetails?.settings?.allow_cancel_job ? 'translate-x-5' : 'translate-x-0',
                              'inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out'
                            )}
                          />
                        </Switch>
