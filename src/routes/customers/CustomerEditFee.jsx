...
const CustomerEditFee = () => {
  ...
  const getFeeInfo = async () => {
    ...
    setSelectedFbos(response2.data.fbos);

    if (response2.data.type === "A") {
      setSelectedAirports(response2.data.travel_fees_airports);
    } else if (response2.data.type === "V") {
      setSelectedAirports(response2.data.vendor_higher_price_airports);
    }

    setAmount(response2.data.fee);
    setSelectedFeeType(feeTypes.find((d) => d.id === response2.data.type));
  ...
  const handleSave = async () => {
    ...
    await api.updateFee(feeId, data);
  ...
  const handleSelectFbo = (value) => {
    if (!isFboSelected(value)) {
      const selectedFbosUpdated = [
        ...selectedFbos,
        fbos.find((el) => el.id === value.id),
      ];

      setSelectedFbos(selectedFbosUpdated);
    } else {
      handleDeselectFbo(value);
    }

    setIsFbosOpen(true);
  };

  const handleDeselectFbo = (value) => {
    const selectedFbosUpdated = selectedFbos.filter((el) => el.id !== value.id);
    setSelectedFbos(selectedFbosUpdated);
    setIsFbosOpen(true);
  };
  ...
  return (
    <AnimatedPage>
      <div className="mx-auto max-w-md">
        <div className="font-medium text-md mb-4">Edit Fee</div>
              ...
              <>
                ...
                <div className="relative">
                  <span className="inline-block w-full rounded-md shadow-sm">
                    <Listbox.Button
                      onClick={() => setIsFbosOpen(!isFbosOpen)}
                      open={isFbosOpen}
...
export default CustomerEditFee;
